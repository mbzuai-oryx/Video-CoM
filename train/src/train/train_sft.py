...
from transformers import (AutoProcessor, BitsAndBytesConfig, HfArgumentParser,
                          ...
                          Qwen2VLForConditionalGeneration, TrainerCallback)

from src.dataset import make_supervised_data_module
from src.dataset.sft_dataset import (DataCollatorForSupervisedDataset,
                                     SupervisedDataset)
from src.params import DataArguments, ModelArguments, TrainingArguments
from src.train.sampler import AlternatingDistributedBatchSampler
from src.trainer import QwenSFTTrainer
from train.train_utils import (get_peft_state_maybe_zero_3,
...
def train():
    ...
    if isinstance(data_args.data_path, str) and "|" in str(data_args.data_path):
        rank, world = get_ddp_rank_world()
        paths = [p.strip() for p in str(data_args.data_path).split("|")]
        datasets = [
            SupervisedDataset(
                data_path=p,
                processor=processor,
        ...
        if data_args.multi_batch_size:
            bs = [
                int(x)
                for x in str(data_args.multi_batch_size).replace(" ", "").split(",")
            ]
        else:
            bs = [training_args.per_device_train_batch_size] * len(datasets)

        if len(bs) != len(datasets):
            raise ValueError("data_args.multi_batch_size must match number of datasets")

        lengths = [len(ds) for ds in datasets]
        print(f"Detected datasets: {datasets}")
        print(f"Lengths of datasets: {lengths}")

        batch_sampler = AlternatingDistributedBatchSampler(
            lengths=lengths,
            batch_sizes=bs,
            world_size=world,
            rank=rank,
        ...
        def _alt_get_train_dataloader(self):
            return alt_loader

        trainer.get_train_dataloader = types.MethodType(
            _alt_get_train_dataloader, trainer
        )

        if hasattr(batch_sampler, "set_epoch"):
            if hasattr(trainer, "add_callback"):

                class _SetEpochCB(TrainerCallback):
                ...
                trainer.add_callback(_SetEpochCB(batch_sampler))
